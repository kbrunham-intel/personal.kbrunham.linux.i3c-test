				   ...
				   u16 payloadlen)
{
	...
	if (payloadlen)
		...
		dest->payload.data = malloc(payloadlen);
	else
...
{
	return i3c_master_rstdaa_locked(master, addr);
}
				     ...
				     struct i3c_dev_desc *dev)
{
	...
	if (master->this_dev != dev && master->ops->attach_i3c_dev) {
...
static void i3c_master_detach_i3c_dev(struct i3c_dev_desc *dev)
{
	...
	if (master->this_dev != dev && master->ops->detach_i3c_dev)
...
i3c_master_register_new_i3c_devs(struct i3c_master_controller *master)
{
	...
	i3c_bus_for_each_i3cdev(&master->bus, desc) {
		if (desc->dev || !desc->info.dyn_addr || desc == master->this_dev)
			continue;

		//desc->dev = kzalloc(sizeof(*desc->dev), GFP_KERNEL);
		desc->dev = malloc(sizeof(*desc->dev));
		if (!desc->dev)
			continue;
		...
		dev_set_name(&desc->dev->dev, "%d-%lx", master->bus.id,
			     desc->info.pid);
			...
			const struct i3c_device_info *info)
{
	...
	master->this_dev = i3cdev;
	master->bus.cur_master = master->this_dev;
...
// 	}
	...
	if (ret && ret != I3C_ERROR_M2) {
		dev_err(&master->dev, "failed to reset dynamic addresses\n");
		//goto err_bus_cleanup;
	}
				  ...
				  u8 addr)
{
	...
	if (newdev->info.dyn_addr != expected_dyn_addr) {
		...
		if (!ret) {
			...
			dev_err(&master->dev,
				"Failed to assign reserved/old address to device %d%lx",
	...
	// 	if (ret) {
	// 		dev_err(&master->dev,
	// 			"Failed to request IBI on device %d-%lx",
	// 			master->bus.id, newdev->info.pid);
	// 	} else if (enable_ibi) {
	// 		ret = i3c_dev_enable_ibi_locked(newdev);
	// 		if (ret)
	// 			dev_err(&master->dev,
	// 				"Failed to re-enable IBI on device %d-%lx",
	// 				master->bus.id, newdev->info.pid);
	// 	}
