...
int i3c_hci_parse_ext_caps(struct i3c_hci *hci)
{
	...
	for (; !err && curr_cap < end; curr_cap += cap_length * 4) {
		u32 curr_cap_u32 = ((u64)curr_cap) & 0xffffffff;
		// KALEN early exit
		if (curr_cap_u32 == 0x0194) {
			break;
		}
		...
		if (cap_id >= 0xc0 && cap_id <= 0xcf) {
			dev_dbg(&hci->master.dev,
				"vendor specific ext_cap 0x%02x for cur_cap 0x%02X\n", cap_id, curr_cap_u32);
		...
		cap_entry = NULL;
		dev_dbg(&hci->master.dev, "Parsing for ext_cap 0x%02x for cur_cap 0x%02X\n", cap_id, curr_cap_u32);
		...
		if (!cap_entry) {
			...
			dev_notice(&hci->master.dev,
				   "parsing ext_cap 0x%02X of length %u using custom parser\n", cap_id, cap_length);
